...
def _construire_portefeuille(db):
    ...
    db.add(Holding(user_id=ID_UTILISATEUR_TEST, ticker="EU1", quantite=1.0, prix_revient_moyen=300.0))
    db.add(MarketDataCache(ticker="EU1", prix_actuel=300.0, region="Europe", derniere_maj=now))
    db.add(Holding(user_id=ID_UTILISATEUR_TEST, ticker="US1", quantite=1.0, prix_revient_moyen=700.0))
    db.add(MarketDataCache(ticker="US1", prix_actuel=700.0, region="Amérique du Nord", derniere_maj=now))
    db.add(AllocationTarget(user_id=ID_UTILISATEUR_TEST, annee=2024, type="geo", categorie="Europe", pourcentage_cible=20.0))
    db.add(AllocationTarget(user_id=ID_UTILISATEUR_TEST, annee=2024, type="geo", categorie="Amérique du Nord", pourcentage_cible=67.0))
    db.commit()
...
def test_aucune_alerte_sans_ecart_au_dessus_du_seuil(client, db):
    now = datetime.now(timezone.utc)
    db.add(Holding(user_id=ID_UTILISATEUR_TEST, ticker="EU1", quantite=1.0, prix_revient_moyen=1000.0))
    db.add(MarketDataCache(ticker="EU1", prix_actuel=1000.0, region="Europe", derniere_maj=now))
    db.add(AllocationTarget(user_id=ID_UTILISATEUR_TEST, annee=2024, type="geo", categorie="Europe", pourcentage_cible=100.0))
    db.commit()
