...
function TransactionImportSection() {
  ...
  return (
    <Card>
      <h3 className="mb-1 text-sm font-semibold text-slate-900">Historique de transactions (format détecté automatiquement)</h3>
      ...
      <input ref={txInputRef} type="file" accept=".csv" onChange={handleFileChange} className="text-sm" />
      {uploading && <p className="mt-2 text-sm text-slate-500">Import et recalcul en cours (peut prendre quelques instants)...</p>}
...
export default function ImportPage() {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-xl font-semibold text-slate-900">Importer le portefeuille</h2>
      ...
      <Card>
        ...
        <input
          ...
          className="text-sm"
        />
        {uploading && <p className="mt-2 text-sm text-slate-500">Lecture du fichier...</p>}
      </Card>

      {error && <p className="text-sm text-red-600">{error}</p>}

      {result && (
        <Card className={result.errors.length > 0 ? 'border-amber-200 bg-amber-50' : 'border-emerald-200 bg-emerald-50'}>
          ...
          <button onClick={() => navigate('/portefeuille')} className="mt-3 text-sm font-medium text-blue-600 hover:underline">
              ...
              <tbody>
                {preview.rows.slice(0, 5).map((row, i) => (
                  <tr key={i} className="border-b border-slate-100">
                    {preview.columns.map((c) => (
                      <td key={c} className="py-1.5 pr-4 text-slate-600">
                        {row[c]}
                      </td>
                    ))}
                  </tr>
