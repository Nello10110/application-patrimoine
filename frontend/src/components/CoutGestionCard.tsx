...
export default function CoutGestionCard({ cout }: { cout: CoutGestionConsolide }) {
  ...
  return (
    <Card title="Coût de gestion annuel estimé (fonds/ETF)">
      <p className="text-3xl font-semibold text-slate-900 dark:text-slate-100">{formatEuro(cout.cout_annuel_estime)}</p>
      <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
        sur {formatEuro(cout.valeur_fonds, 0)} de fonds/ETF détenus, dont {cout.couverture_pct}% avec des frais de gestion connus.
      </p>
      {cout.couverture_pct < 100 && (
        <p className="mt-3 text-xs text-slate-400 dark:text-slate-500">
          Les {formatEuro(cout.valeur_fonds - cout.valeur_fonds_avec_ter_connu, 0)} restants n'ont pas encore de frais de gestion
