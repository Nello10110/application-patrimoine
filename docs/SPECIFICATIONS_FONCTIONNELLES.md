...
### 3.12 Simulateur de patrimoine et indépendance financière

Calcul **pur** (`services/simulation_service.py`), sans dépendance externe ni appel réseau, projeté depuis le patrimoine net **actuel** (§ 3.11) — l'utilisateur ne fournit que ses hypothèses, jamais son patrimoine de départ.

**Projection** (`GET /api/patrimoine/simulation`) : trajectoire annuelle à horizon réglable (1 à 60 ans, préréglages 5/10/20/30 ans à l'écran) par intérêts composés **mensuels** + épargne mensuelle constante — `rendement_annuel_pct` peut être négatif (scénario pessimiste). Présentée explicitement comme une **hypothèse, pas une promesse** : un rendement moyen constant est une simplification, les marchés ne progressent jamais aussi régulièrement dans la réalité.
