...
export function RepartitionEmpilee({
  ...
  /** Déjà triées, du plus grand au plus petit. `valeur` est le libellé formaté
   * (montant ou pourcentage) affiché à droite. */
        ...
        {parts.map((p, i) => {
          const contenu = (
            <>
              ...
              <span className="ml-auto shrink-0 text-[13px] font-semibold text-ink">{p.valeur}</span>
              <span className="w-12 shrink-0 text-right text-xs text-ink4">{p.pourcentage.toFixed(1)}%</span>
            </>
          )
